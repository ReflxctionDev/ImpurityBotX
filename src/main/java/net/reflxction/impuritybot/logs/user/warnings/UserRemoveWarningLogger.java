...
public class UserRemoveWarningLogger extends Logger {
    ...
    public void onWarningRemoved(WarningRemovedEvent event) {
        if (event.getManager().getWarnings(event.getTarget()) > 0) {
            EmbedBuilder builder = new EmbedFactory(new EmbedBuilder())
                    ...
                    .setDescription("**" + event.getExecutor().getName() + "** has removed **" + event.getAmount() + "** warnings from **" + event.getTarget().getName() + "**. Total: **" + event.getManager().getWarnings(event.getTarget()) + "**")
